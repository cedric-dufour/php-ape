...
extends PHP_APE_Type_String
{
  ...
  /** Maximum <I>safe</I> path length
   * @var integer */
  const SAFE_PATH_LENGTH = 1024;
   ...
   * Data length shall NOT exceed the <I>SAFE_PATH_LENGTH</I>.
   * Directory up-browsing characters (e.g. '<SAMP>../</SAMP>') are NOT allowed.</P>
   * <P><B>THROWS:</B> <SAMP>PHP_APE_Type_Exception</SAMP>.</P>
   *
   * @param string $sString Input string
   * @return string
   * @uses SAFE_PATH_LENGTH
   */
  public static function sanitize( $sString = null )
  {
    if( strlen( $sString ) > self::SAFE_PATH_LENGTH )
      throw new PHP_APE_Type_Exception( __METHOD__, 'Path exceeds allowed length; Path: '.$sString );
    if( strpos( $sString, '../' ) !== false or strpos( $sString, '..\\' ) !== false )
      throw new PHP_APE_Type_Exception( __METHOD__, 'Path may not contain directory up-browsing characters; Path: '.$sString );
    return $sString;
  }
