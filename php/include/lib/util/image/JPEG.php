...
/** JPEG, a class to read and write JPEG metadata
 *
 * <PRE>
 * +----------------------------------------------------------------------+
 * | PHP version 4.0                                                      |
 ...
 * @subpackage External_Copyrighted
 */
 ...
 * @subpackage External_Copyrighted
 */
class PHP_APE_Util_Image_JPEG extends PEAR
//class JPEG extends PEAR  // [PHP-APE] Renamed class to comply with PHP-APE auto-loading mechanism
{
    var $_fileName;
    var $_fp = null;
    var $_type = 'unknown';
    ...
    //function JPEG($fileName)  // [PHP-APE] Renamed constructor to match PHP5 standard
    {
        $this->PEAR();
    ...
    /* *********************************************************** */
    // [PHP-APE] Added return status
    function save($fileName = "") {
      if ($fileName == "") {
        $tmpName = $this->_fileName . ".tmp";
        $ok = $this->_writeJPEG($tmpName);
        if ($ok and file_exists($tmpName)) {
          $ok = rename($tmpName, $this->_fileName);
          if( !$ok ) @unlink($tmpName);
        }
      }
      else {
        $ok = $this->_writeJPEG($fileName);
      }
      return $ok;
    }
