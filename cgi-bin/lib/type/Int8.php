...
/** (Signed) 8-bytes integer type
 *
 * <P><B>NOTE:</B> PHP does NOT handle different types of <I>integer</I> data, but other dataspaces DO. Also,
 * PHP is NOT able to handle 8-bytes <I>integer</I> data; those are thus handled as <I>string</I>.</P>
 *
 * @package PHP_APE_Type
 * @subpackage Classes
 */
class PHP_APE_Type_Int8
extends PHP_APE_Type_String
{
  ...
  /** Minimum value
   * @var string */
  const Value_MIN = '-9223372036854775807';

  /** Maximum value
   * @var string */
  const Value_MAX = '9223372036854775807';
  ...
  public static function parseValue( $mValue, $bStrict = true )
  {
    ...
    if( is_null( $mValue ) ) return $bStrict ? null : (string)'0';
    if( !is_numeric( $mValue ) )
      throw new PHP_APE_Type_Exception( __METHOD__, 'Invalid value; Value: '.$mValue );
